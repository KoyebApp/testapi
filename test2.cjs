...
async function runAllTests() {
  ...
  await runTest('CodestralAi - Should return code response', async () => {
    const response = await api.CodestralAi({ text: 'Write hello world in python', model: 'open-codestral-mamba' });
    validateResponse(response);
  });
  ...
  await runTest('ApkpureSearch - Should search apps', async () => {
    const response = await api.ApkpureSearch({ id: 'com.whatsapp' });
    validateResponse(response);
  });
  ...
  await runTest('AptoideSearch - Should search apps', async () => {
    const response = await api.AptoideSearch({ url: 'telegram' });
    validateResponse(response);
  });
  ...
  await runTest('Pinterest - Should download images', async () => {
